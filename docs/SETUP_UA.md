...
У прод-версії тестові функції не зберігаються в `Code.gs`, щоб не тримати зайві entry points у бойовому скрипті.

Якщо тестовий сценарій відпрацював успішно, значить:
...
3. Після цього запустіть меню `Електроцех` → `Надіслати вибрані замовлення`.
4. Скрипт має:
   - сформувати пакет
   - відправити одне повідомлення менеджеру
   - відправити повідомлення інженеру
